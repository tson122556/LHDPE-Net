...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            #
            # # ============================================================
            # # Gradient Monitoring - YOLOv9 Compatible Version (Final)
            # # ============================================================
            #
            # 1) choose correct iteration variable
            # YOLOv9 uses "ni" = number of iterations seen so far
            iteration = ni

            # create buffer once
            if not hasattr(model, 'grad_samples'):
                model.grad_samples = []
                model.dir_samples = []
                model.prev_grad = None
                model.grad_start_iter = 200
                model.grad_max_samples = 300

            # 2) sampling condition
            if iteration >= model.grad_start_iter and len(model.grad_samples) < model.grad_max_samples:
                ...
                for name, p in model.named_parameters():
                    if p.grad is not None:
                        ...
                        count += 1
                avg_grad = total_norm / max(count, 1)
                model.grad_samples.append(avg_grad)
                ...
                if model.prev_grad is not None:
                    cos_sim = torch.nn.functional.cosine_similarity(
                        current_grad, model.prev_grad, dim=0
                    ).item()
                    model.dir_samples.append(cos_sim)

                model.prev_grad = current_grad.clone()

                # ---------- 3. Stop after enough samples ----------
                if len(model.grad_samples) >= model.grad_max_samples:
                    print(">>> Gradient sampling finished. Saving...")

                    import pandas as pd
                    import matplotlib.pyplot as plt
                    import os

                    os.makedirs("out_data_exper", exist_ok=True)

                    # Save csv
                    df = pd.DataFrame({
                        "grad_magnitude": model.grad_samples,
                        "grad_direction": model.dir_samples + [None]
                    })
                    df.to_csv("out_data_exper/lhdpe1_gradient_analysis.csv", index=False)

                    # Save curve
                    plt.figure(figsize=(16,4))
                    plt.subplot(1,2,1)
                    plt.plot(model.grad_samples)
                    plt.title("Gradient Magnitude")
                    plt.subplot(1,2,2)
                    plt.plot(model.dir_samples)
                    plt.title("Gradient Direction")
                    plt.savefig("out_data_exper/lhdpe1_gradient_analysis.png")

                    print("Saved. Exiting training...")
                    raise SystemExit

            # -------------------------------
            # Gradient magnitude logging                                    second part
            # -------------------------------
            total_grad = 0.0
            count = 0
            for name, param in model.named_parameters():
                if param.grad is not None:
                    g = param.grad.detach()
                    total_grad += g.abs().mean().item()
                    count += 1

            avg_grad = total_grad / (count + 1e-8)
            grad_log.append(avg_grad)

            # -------------------------------
            # (可选) Gradient direction logging
            # -------------------------------
            for name, param in model.named_parameters():
                ...
                if name not in prev_grad:
                    prev_grad[name] = g
                    continue

                cosine = torch.nn.functional.cosine_similarity(
                    g.view(-1), prev_grad[name].view(-1), dim=0
                ).item()

                if name not in grad_dir_log:
                    grad_dir_log[name] = []
                grad_dir_log[name].append(cosine)
    ...
    if RANK in {-1, 0}:
        ...
        import json
        with open('grad_magnitude.json', 'w') as f:
            json.dump(grad_log, f)

        with open('grad_direction.json', 'w') as f:
            json.dump(grad_dir_log, f)
        LOGGER.info("Gradient visualization data saved.")
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='Tiny/yolov9-t-or-parallel.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/selfdata.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--project', default=ROOT / 'pk_runs01/train', help='save to project/name')
    parser.add_argument('--name', default='self_data', help='save to project/name')
    ...
    # parser.add_argument('--find-unused-parameters', action='store_true', help='Find unused parameters')
    ...
    # parser.add_argument('--master_port', type=int, default=6008, help='Port used by the master node')
